    ...
    const renderStyles = (
        ...
        if (pane === paneType.TWO) {
            if (
                (orientation === DeviceOrientation.Landscape ||
                    orientation === DeviceOrientation.LandscapeFlipped) &&
                extensionOptions?.isExtendedLandscape &&
                paneRects.length > 1
            ) {
                return Object.assign(
                    ...
                    extendedStyles(
                        paneRects[0],
                        paneRects[0].height + paneRects[1].height
                    ).extendedPaneVertical,
                    defaultConfig?.twoPane?.paneBody!
                );
            } else if (
                (orientation === DeviceOrientation.Portrait ||
                    orientation === DeviceOrientation.PortraitFlipped) &&
                extensionOptions?.isExtendedPortrait &&
                paneRects.length > 1
            ) {
                return Object.assign(
                    {},
                    extendedStyles(paneRects[0], paneRects[0].width * 2)
                ...
                if (isPaneOneExtended()) {
                    return Object.assign(
                        {},
                        twoPaneStyles(paneRects[1]).paneOneExtended,
                        defaultConfig?.twoPane?.paneBody!
                    );
                }
                return Object.assign(
                    {},
                    twoPaneStyles(paneRects[1]).twoPane,
                    defaultConfig?.twoPane?.paneBody!
                );
            }
            return Object.assign(
                {},
                twoPaneStyles(paneRects[0]).twoPaneManual,
                defaultConfig?.twoPane?.paneBody!
            );
        } else {
            if (
                ...
                return Object.assign(
                    ...
                    defaultConfig?.onePane?.paneBody!
                );
            } else if (
                (orientation === DeviceOrientation.Portrait ||
                    orientation === DeviceOrientation.PortraitFlipped) &&
            ...
            return Object.assign(
                {},
                onePaneStyles(paneRects[0]).onePane,
                defaultConfig?.onePane?.paneBody!
            );
        }
    };
...
const onePaneStyles = (paneRects: WindowRect) =>
    StyleSheet.create({
        onePane: {
...
const extendedStyles = (paneRects: WindowRect, length: number) =>
    StyleSheet.create({
        extendedPaneHorizontal: {
            ...StyleSheet.absoluteFillObject,
            backgroundColor: '#f2f2f2',
...
const twoPaneStyles = (paneRects: WindowRect) =>
    StyleSheet.create({
        twoPane: {
            flex: 1,
            ...StyleSheet.absoluteFillObject,
